...
4. 오른손을 키보드 영역 **위쪽**으로 올리면 마우스 모드 (검지로 커서 이동).
5. `ESC` 로 즉시 종료.
...
마우스는 키보드와 완전히 분리된 독립 컨트롤러이며 **ACTIVE 상태에서만** 동작합니다. 오른손 검지가 키보드 영역
위에 있으면(타이핑 중) 마우스는 자동으로 멈춥니다.
...
- 우측 하단 **미니 키보드**(손가락 색으로 선택 키 표시)와 **미니 마우스**(핀치 중 노랑, 클릭 순간 초록, 드래그 주황)
...
**빠른 타이핑 프리셋** (정확도보다 체감 속도):
```bash
python main.py --press-distance 15 --min-down-frames 2
```
...
| 키가 잘 안 눌림 | 손가락을 더 짧고 깊게 누르기, `--press-distance 15 --min-down-frames 2` |
