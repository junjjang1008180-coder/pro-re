...
| `--hand-lock <track\|body\|strict\|size\|off>` | track | 내 손만 인식: track=양손 펴서 등록한 손만 계속 추적 |
...
| 드래그 | 엄지-검지를 붙인 채 손을 이동(12px 이상) **또는 0.4초 꾹 유지** → 떼면 드롭 |

- 핀치는 2프레임 연속 유지돼야 인정(디바운스), 떼는 거리는 붙는 거리의 1.4배(히스테리시스)
- **드래그 중에는** 떼는 거리 1.8배 + 3프레임 연속이어야 드롭: 손을 빨리 움직일 때 엄지가 살짝 벌어져도 끊기지 않음
- 드래그 감도 상수: `DRAG_START_DISTANCE`(12), `DRAG_HOLD_TIME`(0.4초), `DRAG_RELEASE_FACTOR`(1.8) — `config.py`
...
## 6-1. 내 손만 인식 (`--hand-lock`, 기본 `track`)

카메라에 다른 사람 손이 보여도 **내가 등록한 두 손만** 입력에 씁니다.

**방법 (추적 잠금):** 손은 한 프레임 사이에 순간이동하지 않습니다. 그래서 등록한 손에서 **이어지는 손만** 내 손으로 보고,
화면에 **새로 들어온 손은 어디서 들어오든 무시**합니다. 손 크기나 몸이 보이는지와 상관없습니다.
- **등록:** 양손을 펴서 ACTIVE 로 켜는 순간 그 두 손을 잠급니다 (`a` 키로 켰다면 다음에 두 손이 보일 때).
  등록 전에는 가장 크게 보이는 손 2개를 씁니다 (`Owner: not locked - open both palms`).
- **빠르게 움직여도** 따라갑니다 (프레임 사이 50px + 초당 3000px 까지).
- **잠깐 가려졌다가** 2초 안에 사라진 곳 근처(화면 폭의 25% 이내)에서 다시 나타나면 이어서 씁니다.
- **오래 사라졌거나 먼 곳에서 돌아오면** 그 손바닥을 **0.6초 펴 보이면** 다시 내 손으로 등록됩니다
  (`Owner: right hand lost - show palm`).
- 좌/우는 등록 때 기준으로 유지되어, 손을 교차해도 바뀌지 않습니다.
- 무시된 손은 회색 `NOT YOURS` 로 표시됩니다.
- 한계: 내 손이 사라진 직후 2초 안에 그 자리 근처로 다른 사람 손이 들어오면 이어 받을 수 있습니다.
  다른 사람이 손바닥을 펴서 0.6초 보여 주면 비어 있는 자리로 등록됩니다 (의도적으로 하지 않는 한 일어나지 않음).

| 모드 | 동작 |
|---|---|
| `track` (기본) | 추적 잠금 (위 설명) |
| `body` | 몸(어깨-팔꿈치-손목) 인식으로 내 팔에 붙은 손만. 어깨·팔이 보여야 하고, 손을 카메라 앞으로 들면 몸이 가려져 불안정할 수 있음 |
| `strict` | `body` + 내 몸이 안 보이면 손을 쓰지 않음 |
| `size` | 손 크기/위치 기준 |
| `off` | 끔 (가장 큰 손 2개) |

`body`/`strict` 모드에서만 포즈 모델(`models/pose_landmarker_lite.task`, 약 5.8MB)을 받습니다.
...
| 내 손이 `NOT YOURS` 로 표시됨 | 그 손바닥을 0.6초 펴 보이면 다시 등록됨. 또는 양손을 펴서 ACTIVE 전환(재등록) |
| `Owner: ... lost - show palm` | 등록한 손을 놓침. 손바닥을 펴 보이면 되찾음 |
